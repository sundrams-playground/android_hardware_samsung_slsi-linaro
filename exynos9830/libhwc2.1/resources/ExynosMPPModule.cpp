...
bool ExynosMPPModule::isSupportedCompression(struct exynos_image &src)
{
    if (src.compressionInfo.type == COMP_TYPE_AFBC) {
        ...
        if (ExynosMPP::isSupportedCompression(src) == false)
            return false;

        if (mSharedMPP == nullptr) {
            MPP_LOGE("sharedMPP is NULL");
            return false;
        }

        DisplayInfo sharedDisplayInfo = mSharedMPP->mReservedDisplayInfo;
        if (mReservedDisplayInfo.displayIdentifier.id != UINT32_MAX) {
            if (sharedDisplayInfo.displayIdentifier.id != UINT32_MAX) {
                 /* If sharedDisplay is processed before reservedDisplay's operation
                    and two displays cannot support AFBC at the same time by width size restriction,
                    reservedDisplay does not process AFBC using this DPP. */
                if ((mReservedDisplayInfo.displayIdentifier.id < sharedDisplayInfo.displayIdentifier.id) &&
                    ((mReservedDisplayInfo.xres > 2048) || (sharedDisplayInfo.xres > 2048)))
                     return false;
             }
        }
...
bool ExynosMPPModule::isSupportedTransform(struct exynos_image &src)
{
    ...
    case MPP_DPP_VGFS:
        ...
        {
            if ((src.compressionInfo.type == COMP_TYPE_AFBC) && (src.transform != 0))
                return false;
            return true;
        } else {
            return false;
