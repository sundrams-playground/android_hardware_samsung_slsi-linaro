...
class ExynosVirtualDisplay : public ExynosDisplay {
  public:
    ...
    ExynosVirtualDisplay(DisplayIdentifier node);
     ...
     */
    virtual int setWinConfigData(DevicePresentInfo &presentInfo);
    ...
    virtual int32_t getDisplayInfo(DisplayInfo &dispInfo) override {
        ...
        dispInfo.isWFDState = mIsWFDState;
        return ret;
    };

    virtual bool checkDisplayUnstarted();

    virtual void setExternalPlugState(bool state, uint64_t &geometryChanged);

    /**
     * If mIsWFDState is true, VirtualDisplaySurface use HWC
     */
    uint32_t mIsWFDState;
  ...
  protected:
    ...
    bool mIsFirstFrameDisplayed;

    /**
     * If mExternalPlugState is true,
     * virtual display will skip the frame.
     */
    bool mExternalPlugState;
};
