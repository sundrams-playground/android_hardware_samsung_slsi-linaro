...
public:
    ...
    enum setting_check_t {
        ...
        SETTING_DIMENSION = 4,
        SETTING_STRIDE = 8,
        SETTING_MASK_MANDATORY = SETTING_TYPE | SETTING_BUFFER | SETTING_DIMENSION,
        SETTING_MASK = SETTING_TYPE | SETTING_BUFFER | SETTING_DIMENSION | SETTING_STRIDE,
        SETTING_TYPE_MODIFIED = 16,
        SETTING_BUFFER_MODIFIED = 32,
        SETTING_DIMENSION_MODIFIED = 64,
        SETTING_STRIDE_MODIFIED = 128,
        SETTING_MODIFIED_MASK = SETTING_TYPE_MODIFIED | SETTING_BUFFER_MODIFIED |
                                SETTING_DIMENSION_MODIFIED | SETTING_STRIDE_MODIFIED,
    };
    ...
    /*
     * Configure the restriction about alignment of stride
     * This restriction depends on characteristic of HW.
     * But, the buffer will be shared between different HWs.
     * And, every HW should read the buffer with same stride.
     * So, if user gives this, then driver will calculate the stride like this.
     *      stride = ALIGN(width, alignment);
     * If there is no alignment given, then driver will calculate stride by policy of the format.
     */
    bool setStride(uint32_t stride[MAX_HW2D_PLANES]);

    uint32_t getStride(unsigned int index)
    {
        return ((getSettingFlags() & SETTING_STRIDE) != 0) ? mStride[index] : 0;
    }
    /*
    ...
    bool isSettingOkay()
    {
        return (mSettingFlags & SETTING_MASK_MANDATORY) == SETTING_MASK_MANDATORY;
    }
...
private:
    ...
    uint32_t mStride[MAX_HW2D_PLANES];
...
public:
    ...
    bool setCanvasStride(uint32_t stride[MAX_HW2D_PLANES])
    {
        return mCanvas.setStride(stride);
    }
    /*
    ...
    virtual void setLibHdrCoefficient(int __unused *layermap, void __unused *hdrcoef) { };
    virtual void clearLibHdrCoefficient() { };

    const std::vector<std::tuple<uint32_t, uint32_t, uint32_t>> &getTablePPC() const { return mTablePPC; }
protected:
    /*
    ...
    void pushPPC(uint32_t halfmt, uint32_t ppc, uint32_t ppcRot) { mTablePPC.push_back({halfmt, ppc, ppcRot}); }
private:
    ...
    std::vector<std::tuple<uint32_t, uint32_t, uint32_t>> mTablePPC;
};
...
#define ACRYLIC_MAX_FRAMERATE	INT_MAX
