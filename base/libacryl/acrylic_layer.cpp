...
bool AcrylicCanvas::setStride(uint32_t stride[MAX_HW2D_PLANES])
{
    bool modified = false;

    for (int i = 0; i < MAX_HW2D_PLANES; i++) {
        if (((getSettingFlags() & SETTING_STRIDE) != 0) &&
            (mStride[i] == stride[i]))
            continue;
    ...
    if (modified)
        set(SETTING_STRIDE | SETTING_STRIDE_MODIFIED);
...
AcrylicLayer::AcrylicLayer(Acrylic *compositor)
    : AcrylicCanvas(compositor), mTransitData(nullptr), mLayerData(nullptr),
      mLayerDataLen(0), mBlendingMode(HWC_BLENDING_NONE),
      mTransform(0), mZOrder(0), mMaxLuminance(0), mMinLuminance(0), mPlaneAlpha(255)
{
    // Default settings:
