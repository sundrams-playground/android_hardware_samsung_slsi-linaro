    ...
    for (test_type_struct type: test_types) {
        for (unsigned int i = 0; i < getHeapCount(); i++) {
            ...
            for (size_t size : allocation_sizes) {
                ...
                EXPECT_EQ(0, ret = ioctl(getIonFd(), ION_IOC_NEW_ALLOC, &data)) << ": " << strerror(errno);
...
TEST_F(Allocate, InvalidValues)
{
    ...
    EXPECT_EQ(-1, ret = ioctl(0, ION_IOC_NEW_ALLOC, &data));
    EXPECT_TRUE(errno == EINVAL || errno == ENOTTY) << " unexpected error: " << strerror(errno);

    // invalid fd
    EXPECT_EQ(-1, ret = ioctl(-1, ION_IOC_NEW_ALLOC, &data));
    EXPECT_EQ(EBADF, errno) << " unexpected error: " << strerror(errno);

    // invalid heap id
    data.heap_id_mask = 0;
    EXPECT_EQ(-1, ret = ioctl(getIonFd(), ION_IOC_NEW_ALLOC, &data));
