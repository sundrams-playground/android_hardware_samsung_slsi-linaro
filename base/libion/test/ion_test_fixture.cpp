...
void IonTest::SetUp()
{
    ...
    m_ionHeapData = new ion_exynos_heap_data[ION_NUM_HEAP_IDS];
    if (m_ionHeapData != NULL) {
        ion_heap_query query;
        int ret;

        memset(&query, 0, sizeof(query));
        memset(m_ionHeapData, 0, sizeof(ion_exynos_heap_data) * ION_NUM_HEAP_IDS);
                            ...
                            unsigned int flags)
{
    ...
    if (ioctl(getIonFd(), ION_IOC_NEW_ALLOC, &data) == 0)
